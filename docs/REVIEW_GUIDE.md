...
Read [transaction and receipt commitments](BLOCK_TRANSACTION_COMMITMENTS.md) for complete encoded transaction checks, index-based receipt association and the synthetic evidence boundary. Existing history captures omit complete signed transaction bytes and cannot establish this new check by themselves.

Read [block body acquisition](BLOCK_BODY_ACQUISITION.md) for the new live local capture, offline replay, exact raw transaction checks and remaining sender/endpoint limits.

Read [signed body acquisition](SIGNED_BODY_ACQUISITION.md) for the accepted local capture, test-key signing, exact submission-to-acquisition binding, reproduction commands and remaining sender/endpoint limits.

Read [transaction sender verification](BLOCK_SENDER_VERIFICATION.md) for the separate public-key recovery check, native-generated signature vectors, retained local captures and explicit chain/authority limits.
